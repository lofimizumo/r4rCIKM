...
def evaluate_sequence(recommender, seq, evaluation_functions, user, ground_truth_count, look_ahead, top_n):
    """
    ...
    """
    user_profile = seq[:-ground_truth_count]
    ground_truth = seq[-ground_truth_count:]

    # restrict ground truth to look_ahead
    ground_truth = ground_truth[:look_ahead] if look_ahead != 'all' else ground_truth
    # list of list format
    ground_truth = list(map(lambda x: [x], ground_truth))
...
def sequence_sequential_evaluation(recommender, seq, evaluation_functions, user, given_k, look_ahead, top_n, step):
    ...
    for gk in range(given_k, len(seq), step):
        eval_res += evaluate_sequence(recommender, seq,
                                      evaluation_functions, user, gk, look_ahead, top_n)
        eval_cnt += 1
